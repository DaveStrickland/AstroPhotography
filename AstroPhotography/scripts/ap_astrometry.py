...
def command_line_opts(argv):
    ...
    parser = argparse.ArgumentParser(
        prog="ap_astrometry", description="Generate an astrometric solution using astrometry.net"
    )

    # Required
    parser.add_argument("inp_image", help="Input FITS image file without a vaid WCS solution.")
    parser.add_argument(
        "source_list", help="Input FITS table of star positions found by ap_find_stars."
    )
    parser.add_argument("out_image", help="Ouput copy of the FITS image with WCS info (if found).")
    ...
    parser.add_argument(
        ...
        help=(
            "Override the estimate plate scale in the source list file"
    ...
    if args.key is None:
        ...
        if not ast.api_key:
            ...
            parser.error(
                "Your astroquery config file does not contain an Astrometry.net key. You must correct that or supply one using --key."
            )
...
def main(args=None):
    ...
    ap_astrom = ap.ApAstrometry(
        p_inpimg,
        p_srclist,
        p_outimg,
        inp_img_extnum=p_img_extnum,
