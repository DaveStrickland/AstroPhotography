...
def command_line_opts(argv):
    ...
    def_value = defaults["exclude_corner_pct"]
    parser.add_argument(
        "--exclude_corner_pct",
        default=def_value,
        help=(
    ...
    def_value = defaults["combine_type"]
    parser.add_argument(
        "--combine_type",
        default=def_value,
        help=(
            "Swarp method used to combine (stack) multiple images once"
            " resampled to the same image FOV and pixel scale."
            " Must be ene of the valid swarp COMBINE_TYPE values, i.e. one"
            " of AVERAGE, CHI2, MEDIAN, MIN, MAX, SUM, WEIGHTED."
            " It is recommened to include the chosen type in your"
            " --resampled_file_suffix, e.g. for WEIGHTED the suffix would"
            " be --resampled_file_suffix=_resamp_weighted.fits."
            f" Default value: {def_value}"
        ),
    )
    ...
    def_value = defaults["clean_preprocess"]
    parser.add_argument(
        "--no_clean_preprocess",
        default=def_value,
        action="store_false",
        help=(
            "By default preprocessing will overwrite any existing file with"
            " the same name as a preprocessed output file. If this parameter"
            " is specified then the presence of the expcted output file"
            " will cause preprocessing to be skipped for the assciated input file."
        ),
    )
...
def main(args):
    ...
    nav_stat_tbl, stacked_imlist, stacked_im_tbl, preproc_flist = processor.process_all(
        ...
        quiet=p_args.quiet,
        exclude_corner_pct=p_args.exclude_corner_pct,
        ...
        combine_type=p_args.combine_type,
        ...
        clean_preprocess=p_args.no_clean_preprocess,
    )
