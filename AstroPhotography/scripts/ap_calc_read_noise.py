...
def command_line_opts(argv):
    ...
    parser.add_argument(
        ...
        default=p_gain,
        help=(
            "FITS keyword listing the electronic gain (electrons/ADU)"
...
class ApImageDifference:
    ...
    def __init__(self, imdata1, imdata2, sigmaclip, loglevel, mask1=None, mask2=None):
    ...
    def _check_input_images(self, imdata1, imdata2):
        ...
        if imdata1.shape != imdata2.shape:
            err_msg = (
                "Error, data array shapes do not match:"
...
class ApCalcReadNoise:
    ...
    def __init__(
        ...
        self._logger.debug(
            (
                "Initialized an ApCalcReadNoise instance with"
    ...
    def _plot_difference_histogram(self, im_diff, histplot):
        ...
        for ax in [ax1, ax2]:
            plt.sca(ax)
            nperbin, outbins, patches = plt.hist(
                gooddata.ravel(),
                bins=bins,
                density=False,
                weights=None,
            ...
            ax.vlines(
                meanval, ymin, ymax, linestyles="dashed", linewidth=0.8, label="Masked data mean"
            )
            ax.vlines(
                medval, ymin, ymax, linestyles="dotted", linewidth=0.8, label="Masked data median"
            )
...
def main(args=None):
    ...
    read_noise_calculator = ApCalcReadNoise(p_biasfile1, p_biasfile2, p_gain, p_loglevel)
