...
def command_line_opts(argv):
    ...
    parser = argparse.ArgumentParser(
        prog="ap_find_stars", description="Find stars in a FITS image using photutils"
    )

    # Required
    parser.add_argument("fits_image", help="Input FITS image file name to process.")
    parser.add_argument("source_list", help="File name for output FITS table of detected sources.")
    ...
    parser.add_argument(
        ...
        type=float,
        help=(
            "Initial source search threshold in numbers of sigma above the"
            f" background (standard deviations). Default: {def_search_nsigma}"
        ),
    )
    parser.add_argument(
        "--bitdepth",
...
def main(args=None):
    ...
    logging.getLogger(__name__).info(
        f"Elapsed wall clock runtime {perf_time:.3f} s (process time {prcs_time:.3f} s)."
    )
    return 0
