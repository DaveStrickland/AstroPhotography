...
class ApCalibrate:
    ...
    def calibrate(self, raw_image, cal_image, delta_pix, norm_flat, fixcosmic):
        ...
        odict = {
            "BIASCORR": (True, "True if bias subtracted."),
        ...
        if self._master_flat_file is not None:
            img_bdf = np.where(self._norm_flat != 0, img_sub_bd / self._norm_flat, img_sub_bd)
            ...
            if norm_flat is not None:
                ...
                self._write_corrected_image(raw_image, ext_num, norm_flat, self._norm_flat, {})
        else:
        ...
        if self._bpix is not None:
            img_bdf, bpix_odict = self._bpix.fix_bad_pixels(img_bdf, self._mskdata, delta_pix)

            # Update output FITS header keyword dictionary.
            odict["BPIXFILE"] = (self._master_bpix.name, "Name of master bad pixel file used")
        ...
        self._write_corrected_image(raw_image, ext_num, cal_image, img_bdf, odict)
