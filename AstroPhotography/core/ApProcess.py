...
class ApProcess:
    ...
    * ``navfile``: Navigated images are copies of the input calibrated images
    ...
    def get_filter_list(self, ap_filetype='input'):
        ...
        uniq_filter_exposure = unique(ifc_filtertype.summary, keys=['filter', 'exposure'], keep='first', silent=True)
        # TODO The following summary is misleading becuase it lists the file names, which are not really unique
        self._logger.debug(f'Unique filter and exposure time combinations:\n{uniq_filter_exposure.pprint(max_lines=-1, max_width=-1)}')
        filter_list = []
