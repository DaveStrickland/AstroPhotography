...
class ApProcess:
    ...
    * ``navfile``: Navigated images are copies of the input calibrated images
    ...
    def resample_images_to_match(self, target_wcs_file, resampled_file_prefix,
        ...
        Returns
        -------
        stacked_images : list of str
            A list with the file path and name of the resampled images
        ...
        for filter in filter_list:
            ...
            if len(inp_files) == 0:
                ...
                try:
                    ...
                    stacked_images.append( ofilename )
        ...
        # Generate a summary
        self._stacked_images = stacked_images
    ...
    def make_filter_image_summary(self, filter_list, img_file_list, resampled_image_info=False):
        ...
        if resampled_image_info:
            ...
            for idx, filter_name in enumerate(filter_list):
                matching_file_list = self._get_files_matching(self._stacked_images, self._extnum, 'FILTER', filter_name)
                num_matching = len(matching_file_list)
                if num_matching == 0:
                    resampled_imgs.append(f'None')
                elif num_matching == 1:
                    resampled_imgs.append( matching_file_list[0] )
                else:
                    resampled_imgs.append( ','.join(matching_file_list) )
