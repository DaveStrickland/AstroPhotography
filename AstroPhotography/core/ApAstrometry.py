...
from astroquery.astrometry_net import AstrometryNet  # noqa: E402
...
class ApAstrometry:
    ...
    def __init__(
        ...
        if inp_img_fname == out_img_fname:
            err_msg = (
                "Output image cannot be the same as the input image."
        ...
        self._idata, self._ihdr = self._read_fits_image(inp_img_fname, inp_img_extnum)
        self._cols = self._idata.shape[1]
        self._rows = self._idata.shape[0]

        # Read star position data
        self._src_fname = srclist_fname
        self._src_xytable, self._src_meta = self._read_srclist(srclist_fname, srclist_extname)
        ...
        if len(self._wcs) > 0:
            ...
            self._update_sourcelist(self._src_fname, self._out_fname)
        else:
            self._logger.warn(
                f"No output as no WCS found. Output image {out_img_fname} NOT created."
            )
    ...
    def _generate_hints(self, srclist_hdr):
        ...
        if (aprx_ra is not None) and (aprx_dec is not None):
            ...
            self._logger.debug(
                f"Estimated center_ra={aprx_ra:.3f}, center_dec={aprx_dec:.3f}"
                f", radius={radius:.3f} deg."
            )
        else:
            self._logger.warning("Could not estimate center_ra, center_dec, radius.")
        ...
        if (aprx_xpixsiz is not None) and (aprx_ypixsiz is not None):
            ...
            mean_pix_size = math.sqrt(
                (aprx_xpixsiz * aprx_xpixsiz + aprx_ypixsiz * aprx_ypixsiz) / 2
            )
            ...
            self._logger.debug(
                f"Estimated plate scale={mean_pix_size:.3f}"
                f", range={lolim_pix_size:.3f} to {uplim_pix_size:.3f} {scale_units}"
            )
        else:
            self._logger.warning("Could not generate scale hints.")
    ...
    def _read_srclist(self, srclist_fname: str, srclist_extname: str | int):
        """
        ...
        with fits.open(srclist_fname) as hdu_list:
            ...
            if srclist_extname in hdu_list:
                ...
                err_msg = f"{srclist_fname} does not contain expected {srclist_extname} extension."
                self._logger.error(err_msg)
    ...
    def _sanity_check(self, image_filename, srclist_metadata):
        ...
        if key in srclist_metadata:
            ...
            if image_filename in src_img_file:
                ...
                msg = (
                    "Source list NOT generated from the input image"
                    f", but instead from {src_img_file}"
                )
        else:
            msg = (
                f"Source list file lacks {key} keyword."
                f" Cannot assess if it was produced from {image_filename}"
            )
    ...
    def _write_fits_image(self, wcs):
        ...
        bad_keys = ["SIMPLE", "BITPIX", "NAXIS", "EXTEND", "HISTORY", "DATE", "COMMENT"]
    ...
    def create_primary_header(self, kw_dict):
