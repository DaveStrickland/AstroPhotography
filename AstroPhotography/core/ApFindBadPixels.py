...
class ApFindBadPixels:
    ...
    def __init__(self, darkfile: str, sigma: float, loglevel: str):
        """
    ...
    def _generate_sigmaclip_mask(self, data: np.ndarray, sigma: float):
        """
        ...
        self._logger.debug(
            f"Generating a bad pixel mask using sigma={sigma} clipping on"
            " the input image data values."
        )

        # Compute sigma clipped statistics
        mean, med, std = sigma_clipped_stats(data, sigma=sigma, axis=None)
        self._logger.debug(
            f"Sigma-clipped mean={mean:.2f}, median={med:.2f}"
            f", and madstddev={std:.2f} values (ADU)."
        )

        lothresh = med - (sigma * std)
        hithresh = med + (sigma * std)
        self._logger.info(
            f"Good pixels have values between {lothresh:.2f} and {hithresh:.2f} ADU."
        )
    ...
    def _image_stats(self):
        ...
        if self._logger.getEffectiveLevel() == logging.DEBUG:
            ...
            for idx, pct in enumerate(ipctls):
                self._logger.debug(
                    f"{pct:.2f}% of the data has value of {opctls[idx]:.2f} ADU or less."
                )
    ...
    def _read_user_badpix(self, user_badpix_file):
        ...
        with open(user_badpix_file) as bpfile:
            ...
            if "bad_columns" in yobj:
                ...
                self._logger.debug(
                    f"There are {len(badcols)} bad columns in the user-defined badpixel file."
                )
            else:
            ...
            if "bad_rows" in yobj:
                ...
                self._logger.debug(
                    f"There are {len(badrows)} bad rows in the user-defined badpixel file."
                )
            else:
            ...
            if "bad_rectangles" in yobj:
                ...
                self._logger.debug(
                    f"There are {len(badrect)} bad rectangles in the user-defined badpixel file."
                )
            else:
                self._logger.debug(
                    "There were no bad rectangles in the user-defined badpixel file."
                )
    ...
    def add_user_badpix(self, user_badpix_file):
        ...
        self._logger.debug(
            f"Total number of user-defined bad pixels applied to mask: {num_user_bad}"
        )
        return
