...
class ApMeasureStars:
    ...
    def __init__(
        ...
        self._logger.debug(
            f"Size of full source list used for neighbor removal: {len(self._full_srcs)}"
        )
    ...
    def _calculate_boxes(self):
        ...
        col_fmt_dict = {"xmin": "%8d", "xmax": "%8d", "ymin": "%8d", "ymax": "%8d"}
    ...
    def _do_fitting(self):
        ...
        star_mod = models.Gaussian2D(
            amplitude=ampl, x_mean=xpos, y_mean=ypos, x_stddev=sig_x, y_stddev=sig_y, theta=rotang
        )
        ...
        for idx in range(num_stars):
            ...
            var_arr = np.where(self._pixel_array[idx] > 0, self._pixel_array[idx], 1)
            ...
            rchisq = self._calculate_rchisq(
                self._pixel_array[idx], x_grid, y_grid, std_arr, fitted_mod, current_num_free_pars
            )
            ...
            if fit_ok:
                ...
                axrat_err = axrat * math.sqrt(
                    (fwhm_xerr / fwhm_x) ** 2 + (fwhm_yerr / fwhm_y) ** 2
                )
        ...
        overall_axrat_err = overall_axrat * math.sqrt(
            (madstd_fwhm_x / median_fwhm_x) ** 2 + (madstd_fwhm_y / median_fwhm_y) ** 2
        )
        overall_circular = self.is_circular(
            median_fwhm_x, median_fwhm_y, madstd_fwhm_x, madstd_fwhm_y
        )
        ...
        for idx in range(num_stars):
            fwhm_x = self._fit_table["fwhm_x"][idx]
            fwhm_y = self._fit_table["fwhm_y"][idx]
            circular = self.is_circular(fwhm_x, fwhm_y, madstd_fwhm_x, madstd_fwhm_y)
            self._fit_table["circular"][idx] = circular
    ...
    def _do_single_fit(
        self, fitter, input_mod, data_arr, weights_arr, x_grid, y_grid, max_iterations, index
    ):
        """
        ...
        fitted_mod = fitter(
            input_mod, x=x_grid, y=y_grid, z=data_arr, weights=weights_arr, maxiter=max_iterations
        )
    ...
    def _extract_cutouts(self):
        ...
        tmp_arr = np.zeros(numel, dtype=self._img_data.dtype).reshape(num_stars, wdth, wdth)
        ...
        self._logger.debug(
            f"In {num_stars}x{wdth}x{wdth} pixel array min={minval:.2f}, max={maxval:.2f}"
        )
    ...
    def _fit_box_initialization(self):
        ...
        self._logger.debug(
            f"Adopting a fit box width of {self._box_width_pix} pixels"
    ...
    def _plot_fits(self):
        ...
        norm_func = ImageNormalize(
            self._img_data, interval=minmax_interval, stretch=AsinhStretch()
        )
        ...
        fig, ax_arr = plt.subplots(nrows=fig_rows, ncols=fig_cols, sharex=True, sharey=True)
        ...
        for idx in range(num_stars):
            ...
            plt_imgs.append(
                ax_arr[row_idx, col_idx].imshow(cut_out, origin="lower", norm=norm_func)
            )
        ...
        for ax in ax_arr.flat:
            ax.tick_params(axis="both", labelsize=4, direction="in", color="r", length=3)
            ax.label_outer()

        plt.savefig(self._fwhm_plot, dpi=200, bbox_inches="tight")
    ...
    def _select_candidates(self):
        ...
        self._logger.debug(
            f"Center has radius {radius:.1f} pixels centered on x={xcen:.1f}, y={ycen:.1f}"
        )
    ...
    def _measure_neighbors(self):
        ...
        for idx in range(init_size):
            ...
            xypos = [self._init_srcs["xcenter"][idx], self._init_srcs["ycenter"][idx]]
