...
class ApFixHoles:
    ...
    def fix_files(self, inpdata_file, badpixmask_file, outdata_file, deltapix=1):
        ...
        odict["BPIXFILE"] = (Path(badpixmask_file).name, "Name of master bad pixel file used")
    ...
    def fix_bad_pixels(self, data, badpixmask, deltapix=1):
        ...
        self._logger.info(
            f"fix_bad_pixels: data has {data.shape[0]} rows x {data.shape[1]} columns"
            f", dtype={data.dtype}"
        )
        self._logger.info(
            f"fix_bad_pixels: mask has {badpixmask.shape[0]} rows x {badpixmask.shape[1]} columns"
            f", dtype={badpixmask.dtype}"
        )
        ...
        newdata = data.copy()
        mask = badpixmask != ApFixHoles.MASK_GOOD
        npix = data.size
        nbad = np.sum(mask)
        ...
        fixed_stats = {
            "numpix": (npix, "Total number of pixels in image"),
        ...
        for idx, pos in enumerate(zip(bp_row_idxs, bp_col_idxs)):
            ...
            if ngood_co >= self._min_valid:
                ...
                can_fix = True
                newmask[ridx, cidx] = ApFixHoles.MASK_GOOD
                ##if idx < 3:
                ##    print(data_co)
                ##    print(good_mask)
        ...
        msg = f"Processed {nbad} pixels in {run_time:.3f} s, {ms_per_pix:.3f} ms per bad pixel."
        self._logger.info(msg)
        ...
        if nnotfix > 0:
            msg = (
                f"Could not fix {nnotfix} pixels as they had less"
