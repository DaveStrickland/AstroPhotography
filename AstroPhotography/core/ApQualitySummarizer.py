...
class ApQualitySummarizer:
    ...
    def _build_summary_table(self):
        ...
        for ttfkey, ttfval in self._index_dict.items():
            ndata = len(ttfval)
            self._logger.debug(
                f"Processing target:telescope:filter {ttfkey} with {ndata} datatsets."
            )
    ...
    def _find_files(self):
        ...
        if self._walktree:
            in_or_under = "under"
        self._logger.info(
            f'Searching for file matching pattern "{file_pattern}" {in_or_under} {self._qualdir}'
        )
    ...
    def _read_files(self):
            ...
            with path_obj.open(mode="r") as f_handle:
                ...
                if "image_info" in data:
                    ...
                    self._logger.warning(
                        f"Input file {str(path_obj)} lacks image_info data. Skipping this file."
                    )
        ...
        self._logger.info(
            f"There are {num_groups} unique target/telescope/filter groupings: {key_str}"
        )
        return
    ...
    def _write_summary_table(self):
        ...
        ofmt = "ascii.csv"
        over = True
        self._summary_table.write(self._sumfile, format=ofmt, overwrite=over)
