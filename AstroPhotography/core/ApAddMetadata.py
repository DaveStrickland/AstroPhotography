...
class ApAddMetadata:
    ...
    def _get_itelescope_site(self, telescope):
        ...
        mayhill_nm = EarthLocation.from_geodetic(nm_loc["lon"], nm_loc["lat"], nm_loc["elev"])
        nerpio_es = EarthLocation.from_geodetic(es_loc["lon"], es_loc["lat"], es_loc["elev"])
        sidingspring_au = EarthLocation.from_geodetic(au_loc["lon"], au_loc["lat"], au_loc["elev"])
        auberry_ca = EarthLocation.from_geodetic(ca_loc["lon"], ca_loc["lat"], ca_loc["elev"])

        mayhill_obs = Observer(
            name="iTelescope New Mexico",
            location=mayhill_nm,
            description="iTelescope at Mayhill, NM",
        )
        nerpio_obs = Observer(
            name="iTelescope Astrocamp",
            location=nerpio_es,
            description="iTelescope at Nerpio, Spain",
        )
        sidingspring_obs = Observer(
            name="iTelescope Siding Spring",
            location=sidingspring_au,
            description="iTelescope at Siding Spring, Australia",
        )
        auberry_obs = Observer(
            name="iTelescope Sierra Remote",
            location=auberry_ca,
        ...
        if telescope_id.lower() in tel_site_dict:
            ...
            if "mayhill" in loc:
                ...
                err_msg = (
                    f"Error, unexpected location {loc} found for telescope {telescope_id.lower()}."
                )
        else:
            err_msg = (
                f"Error, telescope {telescope_id.lower()} not in list of iTelescope telescopes."
            )
    ...
    def _parse_itelescope_filename(self, filename):
        ...
        if num_fields > 3:
            ...
            mosaic_re = re.compile(r" x\d+ y\d+")
            mosaic_check = mosaic_re.search(target)
            if mosaic_check:
                # Strip off the target part.
                self._logger.debug(
                    f"Mosaic suffix pattern found in file-based target name [{target}]"
                )
                target = target[0 : mosaic_check.start()]
        else:
            err_msg = (
                f"Error, splitting the file name {filename}"
    ...
    def _check_header(self, fitshdr, required_kw_list, optional_kw_list):
        ...
        if len(missing_opt) > 0:
            ...
            self._logger.warning(
                f"FITS header does not contain these optional keywords: {missing_str}"
            )
    ...
    def _write_corrected_header(self, fitsfile, kwdict):
        ...
        with fits.open(
            fitsfile, mode="update", uint=uint_handling, do_not_scale_image_data=image_scaling
        ) as hdu_list:
            # Modify header
    ...
    def process(self, fitsfile, mode, user_target_name=None, yamlfile=None):
        ...
        if "iTelescope" in mode:
            ...
            if user_target_name is not None:
                self._logger.debug(
                    f"User specified target name [{user_target_name}] will be used instead of filename derived target name [{target_str}]."
                )
                target_str = user_target_name

            self._logger.info(
                f"Telescope={telescope_str}, observer={observer_str}, and target={target_str}."
            )
            ...
            for key, val in yaml_dict.items():
                if not np.isscalar(val):
                    self._logger.warning(
                        f"Will not add value to FITS header for key={key} as it is a sequence."
                    )
                else:
                    key_up = key.upper()
        ...
        if "DATE-OBS" in fhdr:
            ...
            self._logger.warning(
                "Can not compute AIRMASS at start of observation without DATE-OBS value."
            )
