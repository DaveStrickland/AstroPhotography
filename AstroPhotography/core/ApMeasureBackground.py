...
class ApMeasureBackground:
    ...
    def _check_file_exists(self, filename: str) -> Any:
    ...
    def _make_source_mask(self):
        ...
        self._logger.debug(
            f"Generated segmentation source mask covering {pct:.3f}% of the image"
            f" ({npos}/{npix} pixels)"
        )
    ...
    def _read_fits(self, image_filename, image_extension):
        ...
        if ndim == 3:
            ...
            info_str = (
                f"{ndim}-D BITPIX={bitpix} image with {cols} columns, {rows} rows,"
                f" {layers} layers, BSCALE={bscale}, BZERO={bzero}"
            )
    ...
    def _set_bgbox_size(self, imrows, imcols, nbg_rows, nbg_cols, min_bgheight, min_bgwidth):
        """
        ...
        self._logger.debug(
            (
                "Background box size constraints:"
                f" {actual_nbg_rows} rows x {actual_nbg_cols} columns of background boxes."
                f" Minimum box size {actual_min_bgheight} pixels high"
                f" x {actual_min_bgwidth} pixels wide."
            )
        )
        ...
        quantum = 2
        box_height = max(
            actual_min_bgheight, quantum * (1 + int(imrows / (quantum * actual_nbg_rows)))
        )
        box_width = max(
            actual_min_bgwidth, quantum * (1 + int(imcols / (quantum * actual_nbg_cols)))
        )
        ...
        if nrows < imrows:
            ...
            if (imrows - nrows) > quantum * actual_nbg_rows:
                self._logger.error(
                    f"Error in _set_bgbox_size logic: missing {imrows-nrows}"
                    f" rows using box height of {box_height} rows."
                )
            else:
                box_height += quantum
        ...
        if ncols < imcols:
            ...
            if (imcols - ncols) > quantum * actual_nbg_cols:
                self._logger.error(
                    f"Error in _set_bgbox_size logic: missing {imcols-ncols} cols"
                    f" using box width of {box_width} cols."
                )
            else:
                box_width += quantum

        self._logger.info(
            f"Background box size is {box_height} pixels high x {box_width} pixels wide."
        )
    ...
    def process_data(
        ...
        *instead* in cases where the simple automated source detection
        ...
        self._set_bgbox_size(
            imdata.shape[0], imdata.shape[1], nbg_rows, nbg_cols, min_bgheight, min_bgwidth
        )

        self._logger.debug(
            (
                "Background estimator settings used: estimator=MedianBackground"
        ...
        self._logger.info(
            f"Estimated median background level: {self._bgmedian:.3f}+/-{self._bgmedian_rms:.3f}"
        )
    ...
    def write_bgimage(self, output_bgfits):
        ...
        if self._bgdata is None:
            raise RuntimeError(
                "Error, you can not write a background image before generating"
                " one using process_data or process_files."
            )
        ...
        # Write to new file
        hdu_list.writeto(output_bgfits, output_verify="ignore", overwrite=True)
        hdu_list.close()
