...
class ApAutoBadcols:
    ...
    def generate_stats_plot(self, plotstatfile: str) -> None:
        ...
        idx = 1
        self._generate_stats_panel(
            ax_arr[idx], self._rowstats, "Along-row statistics and bad rows", "row"
        )

        fig.tight_layout()
        plt.savefig(plotstatfile, dpi=200, bbox_inches="tight")
    ...
    def _generate_stats_panel(
        ...
        ax2.step(
            stats_arr["idx"], medn_sub_mean, where="mid", color="green", alpha=0.7, linewidth=0.75
        )
        ax2.scatter(
            masked_bad_idx,
            masked_bad_med_sub_mean,
            s=1.5,
            c="red",
            marker="o",
            label=f"Bad {coltype}s ({numbad})",
