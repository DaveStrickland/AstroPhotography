...
class RawConv:
    ...
    def _unsupported_colors(self):
        ...
        err_str = "Image colors ({}) not in supported list of {}.".format(
            self._color_desc, self._supported_colors
        )
    ...
    def _subtract_black_levels(self):
        ...
        self._rawim_r = self._safe_subtract(self._rawim_r, self._mask_r, r_bg)
        self._rawim_g1 = self._safe_subtract(self._rawim_g1, self._mask_g1, g1_bg)
        self._rawim_b = self._safe_subtract(self._rawim_b, self._mask_b, b_bg)
        self._rawim_g2 = self._safe_subtract(self._rawim_g2, self._mask_g2, g2_bg)
    ...
    def _read_exif(self, input_file, use_makernote=None):
        ...
        if self._exif_dict is not None:
            logger.info(
                f"Input file {input_file} has {len(self._exif_dict)} items of EXIF metadata."
            )
    ...
    def _safe_subtract(self, data_arr, data_mask, val_to_subtract):
        ...
        logger.warning(
            "Input array has {} pixels ({:.2f}%) less than array we will subract from it.".format(
                num_odd, pct_odd
            )
        )
        if num_odd > 0:
        ...
        return np.subtract(tmp_raw, val_to_subtract, where=data_mask).copy()
    ...
    def _get_whitebalance_from_region(self, wb_method):
        ...
        for itr in zip(
            [sum_r, sum_g1, sum_b, sum_g2],
            [nvalid_r, nvalid_g1, nvalid_b, nvalid_g2],
            ["R", "G1", "B", "G2"],
        ):
            if itr[0] < 0:
                logger.warning(
                    "For band {}, the sum of data within the region is {}".format(itr[2], itr[0])
                )
            elif itr[1] < 1:
                logger.error(
                    "For band {}, the number of valid pixels within the region is {}".format(
                        itr[2], itr[1]
                    )
                )
    ...
    def _get_sum_in_region(self, data_arr, mask_arr, rpix_list):
        ...
        if mask_arr is not None:
            numpix = np.sum(
                mask_arr[rpix_list[0] : rpix_list[1] + 1, rpix_list[2] : rpix_list[3] + 1]
            )
        else:
    ...
    def rgb(
        ...
        if luminance_method not in allowed_methods:
            err_str = "Unexpected luminance calculate method supplied to RawConv.rgb: {}. Allowed methods are: {}".format(
                luminance_method, allowed_methods
            )
            logger.error(err_str)
        else:
        ...
        if "linear" in luminance_method:
            ...
            rgb_im = self._rawpy.postprocess(
                gamma=linear_gamma,
                no_auto_bright=True,
                no_auto_scale=True,
                output_bps=16,
        ...
        minv, maxv, meanv, stdv, medianv, pctiles = self._image_stats(rgb_im, self._ipctls)
        ...
        if renorm:
            ...
            logger.info(
                f"Renormalizing image from range [{raw_min:.2f}, {raw_max:.2f}] to [{min_adu}, {max_adu}] ADU."
            )
            logger.debug(
                f"Original image statistics before renormalization: min={minv} max={maxv} mean={meanv:.2f}+/-{stdv:.2f} median={medianv} ADU."
            )
            dx = raw_max - raw_min
            dy = max_adu - min_adu
            tmp = rgb_im - raw_min
        ...
        if print_stats:
            logger.info(
                f"Image statistics: min={minv} max={maxv} mean={meanv:.2f}+/-{stdv:.2f} median={medianv} ADU."
            )
    ...
    def grey(
        ...
        if luminance_method not in allowed_methods:
            err_str = "Unexpected luminance calculate method supplied to RawConv.grey: {}. Allowed methods are: {}".format(
                luminance_method, allowed_methods
            )
            logger.error(err_str)
        else:
        ...
        if "direct" in luminance_method:
            ...
            rgb_im, exif_dict = self.rgb(luminance_method, subtract_black, wb_method, False, False)
        ...
        if renorm:
            ...
            # recompute statistics.
            minv, maxv, meanv, stdv, medianv, pctiles = self._image_stats(grey_im, self._ipctls)
