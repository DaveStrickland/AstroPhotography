...
class ApFixCosmicRays:
    ...
    def _generate_sigmaclip_mask(self, data, sigma):
        ...
        self._logger.debug(
            f"Generating a bad pixel mask using sigma={sigma} clipping on the input image data values."
        )

        # Compute sigma clipped statistics
        mean, med, std = sigma_clipped_stats(data, sigma=sigma)
        self._logger.debug(
            f"Sigma-clipped mean={mean:.2f}, median={med:.2f}, and madstddev={std:.2f} values (ADU)."
        )
    ...
    def process(self, inpdata, gain):
        ...
        # Run the algorithm.
        tmp_crimg, crmask = ccdproc.cosmicray_lacosmic(inpdata, **settings_dict)
        ...
        kw_dict = {
            "CR_CLEAN": (True, "Has cosmic ray removal been performed?"),
        ...
        self._logger.debug(
            f"Finished CR processing in {run_time_secs:.3f} s, {ms_per_pix:.3f} ms per CR pixel."
        )
    ...
    def process_file(self, inpfile, outfile):
        ...
        if gain is None:
            ...
            self._logger.warning(
                f"Could not find gain value in header. Assuming gain={gain:.3f} e/ADU."
            )
        ...
        hdu = fits.PrimaryHDU(data=self._cleandata, header=self._imhdr)
